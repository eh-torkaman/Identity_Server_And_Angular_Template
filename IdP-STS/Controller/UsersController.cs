    ...
    public class UsersController : ControllerBase
    {
        ...
        public IActionResult CreateUser(UserForCreationDto us)
        {
            try
            {
                ...
                if (user != null)
                {
                    var rs = new CustomMessages() { new CustomMessage { Message = "این کاربر قبلاً وجود دارد", IsError = true } };
        ...
        public IActionResult ResetUserPassword(UserForCreationDto us)
        {
            try
            {
                ...
                {
                    var rs = new CustomMessages() { new CustomMessage { Message = "برای این کاربر شدنی نیست", IsError = true } };
        ...
        public IActionResult DeleteUser(string userName)
        {
            try
            {
                ...
                {
                    var rs = new CustomMessages() { new CustomMessage { Message = "این کاربر قابل پاک شدن نیست", IsError = true } };
        ...
        public IActionResult LockUser(string userName)
        {
            try
            {
                ...
                else
                {
                    if (userName.ToLower() == "SuperAdmin".ToLower())
                    {
                        var rs = new CustomMessages() { new CustomMessage { Message = "این کاربر قابل قفل شدن نیست", IsError = true } };
                        return BadRequest(rs); 
                    }
